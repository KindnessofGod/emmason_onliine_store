...
import { Rating, SectionHeading, TrustPill, VerifiedBadge } from "@/components/ui";
import {
  categories,
  newArrivals,
  onSaleProducts,
...
export default async function HomePage({
  ...
  const deals = onSaleProducts(8);
  const fresh = newArrivals(4);
  const sellers = verifiedSellers();
          ...
          {categories.map((category) => (
            ...
            >
              ...
              <p className="relative mt-3 flex items-center gap-1 text-xs font-bold">
                {pluralize(productCountByCategory(category.slug), dict.seller.productCountOne, dict.seller.productCount)}
            ...
            {sellers.map((seller) => (
              ...
              >
                ...
                <p className="mt-3 text-xs font-semibold text-brand-700">
                  {pluralize(productCountBySeller(seller.id), dict.seller.productCountOne, dict.seller.productCount)}
                </p>
              </Link>
            ))}
          </div>
