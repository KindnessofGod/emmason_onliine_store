...
export function CheckoutForm({
  ...
  function submit(event: FormEvent) {
    ...
    // No order backend yet: the reference is derived locally so the confirmation
    // is real and stable. Post to the orders API here once it exists.
    const reference = orderReference(Date.now() % 60466176);
    setPlaced({ reference, phone: values.phone });
    clear();
  }
