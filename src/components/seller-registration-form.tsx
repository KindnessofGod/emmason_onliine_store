...
import { submitSellerApplication } from "@/actions/seller";
...
export function SellerRegistrationForm({
  ...
  function submit(event: FormEvent) {
    ...
    startTransition(async () => {
      ...
      // The NIN is masked before it leaves the form, so the raw value never
      // reaches the confirmation screen, the database or a log.
      setSubmitted({
  ...
  return (
    <form onSubmit={submit} noValidate className="space-y-8">
      ...
      <section className="rounded-card border border-ink-100 bg-white p-6 shadow-soft">
        ...
        >
          {pending ? `${dict.seller.submit}…` : dict.seller.submit}
        </button>
